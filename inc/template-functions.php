<?php
/**
 * Functions which enhance the theme by hooking into WordPress
 *
 * @package globalkeys
 */

/**
 * Adds custom classes to the array of body classes.
 *
 * @param array $classes Classes for the body element.
 * @return array
 */
function globalkeys_body_classes( $classes ) {
	// Adds a class of hfeed to non-singular pages.
	if ( ! is_singular() ) {
		$classes[] = 'hfeed';
	}

	// Adds a class of no-sidebar when there is no sidebar present.
	if ( ! is_active_sidebar( 'sidebar-1' ) ) {
		$classes[] = 'no-sidebar';
	}

	// Page has front-page sections (Hero, Intro, CTA etc.).
	if ( function_exists( 'globalkeys_has_front_page_sections' ) && globalkeys_has_front_page_sections() ) {
		$classes[] = 'has-front-page-sections';
	}

	// My Account Login/Register: Sidebar und Footer ausblenden.
	if ( function_exists( 'is_account_page' ) && is_account_page() && ! is_user_logged_in() ) {
		$classes[] = 'gk-account-login';
	}
	// E-Mail-Verifizierungsseite: eigenes Clean-Layout, Header/Footer ausgeblendet.
	if ( get_query_var( 'gk_verify' ) ) {
		$classes[] = 'gk-verify-page';
	}

	return $classes;
}
add_filter( 'body_class', 'globalkeys_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function globalkeys_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'globalkeys_pingback_header' );
