<?php
/**
 * Template part for Weekly Deals Section
 *
 * 1:1 wie Pre-orders: gleiche Überschrift (mit Hover), gleiche Produktliste.
 * Zeigt Produkte aus der Kategorie "weekly-deals".
 *
 * @package globalkeys
 */

$section    = get_query_var( 'gk_section', array( 'id' => 'section-weekly-deals', 'aria_label' => __( 'Weekly deals', 'globalkeys' ) ) );
$id         = ! empty( $section['id'] ) ? $section['id'] : 'section-weekly-deals';
$aria_label = ! empty( $section['aria_label'] ) ? $section['aria_label'] : __( 'Weekly deals', 'globalkeys' );
...
<section id="<?php echo esc_attr( $id ); ?>" class="gk-section gk-section-weekly-deals" role="region" aria-labelledby="<?php echo esc_attr( $id ); ?>-title">
	<div class="gk-section-inner gk-section-featured-inner">
		<div class="gk-featured-heading-wrap">
			<h2 id="<?php echo esc_attr( $id ); ?>-title" class="gk-section-title gk-featured-heading">
				<span class="gk-featured-heading-text-wrap">
					<span class="gk-featured-heading-text"><?php esc_html_e( 'Weekly deals', 'globalkeys' ); ?></span>
		...
		<?php else : ?>
			<p class="gk-section-text gk-featured-empty"><?php esc_html_e( 'Aktuell sind keine Weekly deals hinterlegt.', 'globalkeys' ); ?></p>
		<?php endif; ?>
	</div>
</section>
